...
#claw_motor.run_time(1000,2300, wait = True)
cal.run()
...
while True:
    ...
    if lf.isOffPath():
        robot.drive(50, 0)
    #if line_sensor.reflection() == lf.isOnPath():
    if lf.isOnPath():
        robot.straight(60) # When it finds the path, it will drive 2,5 cm divided by cos(45 degrees) = 3,5 cm.
